...
def imageDiff(resultRoot, originFile, contrastFile, page=1):
    ...
    # 图像合并，生成对比图
    # 新添加整合结果为一张图片
    new_Image.paste(contrastImage, (0, i * contrastSize[1]))
    '''
...
if __name__ == '__main__':
    ...
    contrastImage = Image.open(os.path.join(contrastImagePath, str(1) + '.png'))
    # 新添加整合结果为一张图片
    contrastSize = contrastImage.size  # 获取扫描件大小
    newImage_width = contrastSize[0]
    newImage_hight = contrastSize[1] * contrastImageNum
    new_Image = Image.new('RGB', (newImage_width, newImage_hight), "#000000")
    # 新添加整合结果为一张图片
    if originImageNum != contrastImageNum:
        ...
        #展示扫描图片
        #contrastImage.show()
        new_Image.save(os.path.join(resultRoot,'对比结果.png'))
